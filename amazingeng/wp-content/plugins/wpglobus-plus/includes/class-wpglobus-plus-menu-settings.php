...
if ( ! class_exists( 'WPGlobusPlus_Menu_Settings' ) ) :
	...
	class WPGlobusPlus_Menu_Settings {
		 ...
		 * @var string
		 */
		static public $current_menu_location = '';
		 ...
		 * @var string
		 */
		static public $default_menu_location = '';
		 ...
		 * @var int
		 */
		static public $default_menu_id = 0;
		 ...
		 * @var array
		 */
		static public $menu_locations = array();
		 ...
		 * @var string
		 */
		static public $option_key = '_wpglobus_plus_location_';
		 ...
		 * @var string[]
		 */
		static public $child_parent = array();
		 ...
		 * @return bool
		 */
		static public function filter__add_selector( $disable_add_selector, $args ) {
			...
			if ( ! empty( self::$child_parent[ $opt ] ) ) {
				...
				if ( self::$child_parent[ $opt ] !== WPGlobus::Config()->nav_menu ) {
		 ...
		 */
		static public function filter__nav_menu_args( $args ) {
		 ...
		 */
		static public function ajax_events(
		 ...
		 */
		static public function filter__enable_page( $enabled_pages ) {
			/**
		 ...
		 * Add hidden submenu
		 */
		static public function action__admin_menu() {
...
//			}
			...
			if ( isset( $_GET['location'] ) ) {
				self::$current_menu_location = $_GET['location'];
			} else {
		...
		static public function content() {
			...
			<div class="wrap">
				...
				if ( empty( self::$nav_menus ) ) {
					...
					<div class="notice notice-warning">
						<p>
							 ...
							 */
							echo sprintf( __( 'No menus have been created yet. <a href="%s">Create some</a>.' ), esc_attr( admin_url( 'nav-menus.php' ) ) ); ?>
						</p>
					</div>
					<?php
					echo '</div><!-- .wrap -->';
				...
				if ( empty( $registered_theme_locations )
				     || empty( self::$menu_locations )
				) {
					?>
					<div class="notice notice-warning">
				...
				?>
				<strong><?php esc_html_e( 'Theme Location' ); // No text domain. Standard WP text. ?>:</strong>
				<select id="wpglobus-theme-location" onchange="WPGlobusPlusMenuSettings.selectLocation( jQuery('#wpglobus-theme-location option:selected').val() );">
					<?php foreach ( self::$locations as $menu_location => $menu_title ) {
						$selected = ( self::$current_menu_location === $menu_location ) ? 'selected' : ''; ?>
						<option <?php echo $selected; ?> value="<?php echo $menu_location; ?>"><?php echo $menu_location . ' (' . $menu_title . ')'; ?></option>
					<?php } ?>
				</select>
					...
					<table class="widefat" style="margin: 1em 0">
						...
						<tr>
							...
							foreach ( WPGlobus::Config()->enabled_languages as $lang ) { ?>
								<th><?php echo WPGlobus::Config()->en_language_name[ $lang ]; ?></th>    <?php
							} ?>
						</tr>
						</thead>
						<tbody>    <?php
						foreach ( self::$nav_menus as $menu ) :
//							$_selected_menu_id 	= array();
							$_location = '';
							foreach ( self::$menu_locations as $menu_location => $menu_id ) :
							...
							?>
							<tr class="">
								<td class="" style="<?php echo $style['cells-background-color']; ?>">
									<a target="_blank" href="<?php echo admin_url() . 'nav-menus.php?action=edit&menu=' . $menu->term_id; ?>"><?php echo wp_html_excerpt( $menu->name, 40, '&hellip;' ) . $_location; ?></a>
								</td>
								<?php
								foreach ( WPGlobus::Config()->enabled_languages as $lang ) {
									...
									if ( $lang !== WPGlobus::Config()->default_language ) {
									...
									$cell_style = $style;
									?>
									<td title="<?php echo $cell_title; ?>"
									    id="<?php echo self::$current_menu_location . '-' . $lang . '-' . $menu->term_id; ?>"
									    class="<?php echo implode( ' ', $cell_classes ); ?>"
									    data-language="<?php echo $lang; ?>"
									    data-menu-id="<?php echo $menu->term_id; ?>"
									    data-current-menu-location="<?php echo self::$current_menu_location; ?>"
									    style="<?php echo implode( ';', $cell_style ); ?>">
										<?php echo $_location; ?>
									</td>    <?php
								} ?>
							</tr>    <?php
					...
					<form id="wpglobus-plus-menu-setting-form">
						...
						<input id="wpglobus_plus_menu_location" name="wpglobus_plus_menu_location" value="<?php echo self::$current_menu_location; ?>" type="hidden" />

						<input id="wpglobus_plus_menu_id_<?php echo WPGlobus::Config()->default_language; ?>"
						       name="wpglobus_plus_menu_id_<?php echo WPGlobus::Config()->default_language; ?>"
						       value="<?php echo self::$default_menu_id; ?>"
						       type="hidden" />

						<?php foreach ( WPGlobus::Config()->enabled_languages as $lang ) : ?>
							<?php if ( $lang !== WPGlobus::Config()->default_language ) {
								...
								<input id="wpglobus_plus_menu_id_<?php echo $lang; ?>"
								       name="wpglobus_plus_menu_id_<?php echo $lang; ?>"
								       value="<?php echo $value; ?>"
								       type="hidden" />
							<?php } ?>
						<?php endforeach; ?>

						<button id="wpglobus_menu_settings_save" type="button" class="button-primary" value="save" formnovalidate><?php esc_html_e( 'Save Changes' ); ?></button>
					</form>
					<div class="notice">
						<p>
							<a href="<?php echo admin_url( 'nav-menus.php' ); ?>"><?php esc_html_e( 'Edit Menus' ); ?></a>
							|
							<a href="<?php echo admin_url( 'nav-menus.php?action=locations' ); ?>"><?php esc_html_e( 'Manage Locations' ); ?></a>
						</p>
					</div>
				<?php else: ?>
					<div class="notice notice-warning">
		 ...
		 * @return string|null
		 */
		static public function get_options( $location = '', $language = '' ) {
		...
		static public function enqueue_scripts( $hook_page ) {
			...
			if ( 'admin_page_' . self::MENU_SLUG === $hook_page || 'nav-menus.php' === $pagenow ) {
				...
				wp_register_script(
					'wpglobus-plus-menu-settings',
					plugin_dir_url( __FILE__ ) . 'js/wpglobus-plus-menu-settings' . self::$_SCRIPT_SUFFIX . '.js',
