<?php
	...
	*/
	...
	//имя и почта отправителя
        function res_fromemail($email) { return 'news@amazinghiring.com'; }
	function res_fromname($name){ return 'Даша из Amazinghiring'; }
...
//$myYoutube = get_post_meta($post->ID, 'mycheckbox', true);
 ...
 //$table_item = $wpdb->prefix . "ebd_item";
  // $wpdb->update( $table_item, array("youtube_link"=>$youtube_link));  
//  $wpdb->update_row( $wpdb->prepare( "UPDATE `youtube_link` FROM {$table_item} WHERE `id` = %d LIMIT 1;", $product_id ) );
		    } // end foreach  
		}  
...
//echo $youtube_link;							
		                switch($field['type']) {  
							...
							case 'text':  
							    echo '<input type="text" name="'.$field['id'].'" id="'.$field['id'].'" value="'.$meta.'" size="30" />
							        <br /><span class="description">'.$field['desc'].'</span>';  
									if($field['id'] == 'wp_file_id'){$file_idss = $meta;}
									if ($field['id'] == 'youtube_link')
									{
										$youtube_link = $meta;
...
global $wpbd;
	...
	if ( ! function_exists( 'press_section' ) ) {
 
		// Опишем требуемый функционал
	    function press_section() {
	 
	        $labels = array(
	            'name'                => _x( 'Пресса', 'Post Type General Name', 'press_web' ),
	            'singular_name'       => _x( 'Пресса', 'Post Type Singular Name', 'press_web' ),
	            'menu_name'           => __( 'Пресса', 'press_web' ),
	            'parent_item_colon'   => __( 'Родительский:', 'press_web' ),
	            'all_items'           => __( 'Все записи', 'press_web' ),
	            'view_item'           => __( 'Просмотреть', 'press_web' ),
	            'add_new_item'        => __( 'Добавить новую запись', 'press_web' ),
	            'add_new'             => __( 'Добавить', 'press_web' ),
	            'edit_item'           => __( 'Редактировать запись', 'press_web' ),
	            'update_item'         => __( 'Обновить запись', 'press_web' ),
	            'search_items'        => __( 'Найти запись', 'press_web' ),
	            'not_found'           => __( 'Не найдено', 'press_web' ),
	            'not_found_in_trash'  => __( 'Не найдено в корзине', 'press_web' ),
	        );
	        $args = array(
	            'labels'              => $labels,
	            'supports'            => array( 'title', 'editor', 'excerpt', ),
	            'taxonomies'          => array( 'press_web_tax' ), // категории, которые мы создадим ниже
	            'public'              => true,
	            'menu_position'       => 5,
	            'menu_icon'           => 'dashicons-book',
	        );
	        register_post_type( 'press_web', $args );
	 
	    }	    
	 
	    add_action( 'init', 'press_section', 0 ); // инициализируем

	    function press_meta_box() {  
		    add_meta_box(  
		        'press_meta_box', // Идентификатор(id)
		        'Дополнительная информация', // Заголовок области с мета-полями(title)
		        'show_my_press_metabox', // Вызов(callback)
		        'press_web', // Где будет отображаться наше поле
		        'normal',
		        'high'
	        );
		...
		add_action('add_meta_boxes', 'press_meta_box'); // Запускаем функцию

		$press_meta_fields = array(
		    array(  
		        'label' => 'Ссылка',  
		        'desc'  => 'http://...',
		        'id'    => 'press_link', // даем идентификатор.
		        'type'  => 'text'  // Указываем тип поля.
		    ),
		    array(
			    'name'  => 'Image',
		...
		function show_my_press_metabox() {  
		global $press_meta_fields; // Обозначим наш массив с полями глобальным
		global $post;  // Глобальный $post для получения id создаваемого/редактируемого поста
		// Выводим скрытый input, для верификации. Безопасность прежде всего!
		echo '<input type="hidden" name="custom_meta_box_nonce" value="'.wp_create_nonce(basename(__FILE__)).'" />';  
		    ...
		    // Начинаем выводить таблицу с полями через цикл
		    echo '<table class="form-table">';  
		    foreach ($press_meta_fields as $field) {  
		        // Получаем значение если оно есть для этого поля
		        $meta = get_post_meta($post->ID, $field['id'], true);  
		        // Начинаем выводить таблицу
		        echo '<tr>
		...
		function save_my_press_meta_fields($post_id) {  
		    global $press_meta_fields;  // Массив с нашими полями
		...
		add_action('save_post', 'save_my_press_meta_fields'); // Запускаем функцию сохранения
	...
	/*Убрать автотег в шаблонах*/
	if (!is_page_template('page-press.php') || !is_page_template('page-empty.php')) {
